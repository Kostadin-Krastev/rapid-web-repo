function syncFromExternalSheet() {
  ...
  data = data.filter((row) => row[0] && row[0].toString().trim() !== ''); // премахвам празните редове в таблицата

  for (let i = 0; i < data.length; i++) {
    data[i][1] = replaceStockStatus(data[i][1]);
  }

  data.sort(function (tableRowA, tableRowB) {
    const rocA = parseFloat(tableRowA[0]) || 0;
    const rocB = parseFloat(tableRowB[0]) || 0;
    return rocA - rocB;
  });
